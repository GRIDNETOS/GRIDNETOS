  ...
  struct worker {
    ...
    void operator()() {
      for (;;) {
        ...
        assertGN(true == static_cast<bool>(fn));
        fn();
      }
    }
...
private:
  void append(char c) {
    if (fixed_buffer_used_size_ < fixed_buffer_size_ - 1) {
      ...
      if (glowable_buffer_.empty()) {
        assertGN(fixed_buffer_[fixed_buffer_used_size_] == '\0');
...
inline bool process_server_socket_core(socket_t sock,
                                       ...
                                       T callback) {
  assertGN(keep_alive_max_count > 0);
  auto ret = false;
...
inline bool compress(std::string &content) {
  ...
  do {
    ...
    assertGN(ret != Z_STREAM_ERROR);
    compressed.append(buff.data(), buff.size() - strm.avail_out);
  } while (strm.avail_out == 0);

  assertGN(ret == Z_STREAM_END);
  assertGN(strm.avail_in == 0);
...
inline void Server::stop() {
  if (is_running_) {
    assertGN(svr_sock_ != INVALID_SOCKET);
