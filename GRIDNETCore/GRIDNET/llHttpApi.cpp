...
#define SPAN_CALLBACK_MAYBE(PARSER, NAME, START, LEN)                         \
  do {                                                                        \
    ...
    err = settings->NAME((PARSER), reinterpret_cast<const char*>((START)), (LEN));                           \
    if (err == -1) {                                                          \
      err = HPE_USER;                                                         \
      llhttp_set_error_reason((PARSER), reinterpret_cast<const char*>("Span callback error in " #NAME));     \
    }                                                                         \
  } while (0)
...
void llhttp_reset(llhttp_t* parser) {
    llhttp_type_t type = static_cast<llhttp_type_t>(parser->type);
    const llhttp_settings_t* settings = reinterpret_cast<llhttp_settings_t*>(parser->settings);
...
llhttp_errno_t llhttp_execute(llhttp_t* parser, const char* data, size_t len) {
    return static_cast<llhttp_errno_t>(llhttp__internal_execute(parser, data, data + len));
}
...
llhttp_errno_t llhttp_finish(llhttp_t* parser) {
    ...
    if (parser->error != 0) {
        return static_cast<llhttp_errno_t>(0);
    }

    switch (parser->finish) {
    case HTTP_FINISH_SAFE_WITH_CB:
        CALLBACK_MAYBE(parser, on_message_complete);
        if (err != HPE_OK) return static_cast<llhttp_errno_t>(err);
...
llhttp_errno_t llhttp_get_errno(const llhttp_t* parser) {
    return static_cast<llhttp_errno_t>(parser->error);
}
...
void llhttp_set_error_reason(llhttp_t* parser, const  char* reason) {
    parser->reason = reinterpret_cast<const char*>(reason);
}
...
void llhttp__debug(llhttp_t* s, const  unsigned char* p, const  unsigned char* endp,
    const char* msg) {
    if (p == endp) {
