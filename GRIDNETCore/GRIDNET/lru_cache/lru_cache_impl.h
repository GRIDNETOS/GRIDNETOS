...
struct LinkedList {
  ...
  const Value& replace_value(IndexType index,
      Value new_value) {
      node_type *current_node = &list_content_[index];// index_of(key);
      const Key key = current_node->key();
    
      //node_type& one_before_last = list_content_[oldest_node.prev_];
      //oldest_ = oldest_node.prev_;
      //IndexType oldest_node_index = one_before_last.next_;

      //cache the surroundings before the current node gets delted
      IndexType previous_node = current_node->prev_;
      IndexType next_node = current_node->next_;
      //confirmed, the current node gets deleted below.
      bool wasLatest = (latest_ == index);
      
      auto new_index = list_content_.replace_entry(
          current_node, key,
          node_type{ key, std::move(new_value), previous_node, next_node});


      //list_content_[current_node->prev_].next_ = new_index;
      if (previous_node != INVALID_INDEX)
      {
          list_content_[previous_node].next_ = new_index;
      }

      if (next_node != INVALID_INDEX)
      {
          list_content_[next_node].prev_ = new_index;
      }
...
class LruCacheImpl {
 public:
  ...
  const Value& update(const Key& key, Value new_value) {
      //if (max_size() == size()) {
      //    // Cache is full, drop the last entry and replace it.
      //    return replace_oldest_entry(key, std::move(new_value));
     // }
      ...
      if constexpr (ByAccessOrder) {
          // Update the last access order.
          return value_list_.replace_value(index, new_value);
      }
