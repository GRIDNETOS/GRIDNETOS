...
static inline __m512i enc_reshuffle(const __m512i input) {
    ...
    // in = [D1 D2 D0 D1|C1 C2 C0 C1|B1 B2 B0 B1|A1 A2 A0 A1] x 4
    ...
    // FIX: Extract 6-bit values using mask and multiply operations
    // (This step was missing - it converts 3 bytes into 4 x 6-bit values)
    // Same technique as AVX2 enc_reshuffle in fastavxbase64.cpp
...
size_t fast_avx512bw_base64_decode(char *out, const char *src, size_t srclen) {
  ...
  // If input ends with padding ('='), we need to leave the padded block for scalar decoder
