...
namespace {
	...
	public:
		...
		Botan::fe EdwardsToMontgomeryX(Botan::fe  edwardsY, Botan::fe  edwardsZ);
		Botan::fe MontgomeryXToEdwards(Botan::fe montgomeryX, Botan::fe montgomeryZ);
		std::vector<uint8_t>  EdwardsToMontgomery(std::vector<uint8_t> edwards);
		std::vector<uint8_t>  MontgomeryToEdwards(std::vector<uint8_t> montgomery);
	};


	Botan::fe keyConverter::EdwardsToMontgomeryX(Botan::fe  edwardsY, Botan::fe  edwardsZ)
	{
		// montgomeryX = (edwardsZ + edwardsY) / (edwardsZ - edwardsY)
		Botan::fe montgomeryX;
		Botan::fe tempX, tempZ;
		Botan::fe_add(tempX, edwardsZ, edwardsY);
		Botan::fe_sub(tempZ, edwardsZ, edwardsY);
		Botan::fe_invert(tempZ, tempZ);
		Botan::fe_mul(montgomeryX, tempX, tempZ);
		return montgomeryX;
	}

	Botan::fe keyConverter::MontgomeryXToEdwards(Botan::fe montgomeryX, Botan::fe montgomeryZ)
	{
		// edwardsY = (montgomeryX - montgomeryZ) / (montgomeryX + montgomeryZ)
		Botan::fe edwardsY;
		Botan::fe tempY, tempZ;
		Botan::fe_sub(tempY, montgomeryX, montgomeryZ);
		Botan::fe_add(tempZ, montgomeryX, montgomeryZ);
		Botan::fe_invert(tempZ, tempZ);
		Botan::fe_mul(edwardsY, tempY, tempZ);
		return edwardsY;
	}

	std::vector<uint8_t>  keyConverter::EdwardsToMontgomery(std::vector<uint8_t> edwards)
	{
		std::vector<uint8_t> montgomery;
		montgomery.resize(32);

		Botan::fe edwardsY, edwardsZ, montgomeryX;
		Botan::fe_frombytes(edwardsY, edwards.data());
		Botan::fe_1(edwardsZ);
		montgomeryX = EdwardsToMontgomeryX(edwardsY, edwardsZ);
		Botan::fe_tobytes(montgomery.data(), montgomeryX);
		montgomery[31] |= (edwards[31] & 0x80);// copy sign
		return montgomery;
	}

	std::vector<uint8_t>  keyConverter::MontgomeryToEdwards(std::vector<uint8_t> montgomery)
	{
		std::vector<uint8_t> edwards;
		edwards.resize(32);

		Botan::fe montgomeryX, montgomeryZ, edwardsY;
		Botan::fe_frombytes(montgomeryX, montgomery.data());
		Botan::fe_1(montgomeryZ);
		edwardsY = MontgomeryXToEdwards(montgomeryX, montgomeryZ);
		Botan::fe_tobytes(edwards.data(), edwardsY);
		edwards[31] |= (montgomery[31] & 0x80);// copy sign
