...
protected:
	virtual void CombineMessageAndShiftRegister(byte *output, byte *mRegisters, const byte *message, size_t length) =0;
...
{
	bool IsForwardTransformation() const {return true;}
	void CombineMessageAndShiftRegister(byte *output, byte *mRegisters, const byte *message, size_t length);
};
