...
#define MG_MAX_RECV_BUF_SIZE \
  (10 * 1024 * 1024)  // 10MB instead of default 128KB
#endif

#ifndef MG_IO_SIZE
#define MG_IO_SIZE (64 * 1024)  // 64KB chunks instead of default 2KB
#endif
...
#include <string.h>
#define IOBUF_ALIGNMENT 8  // Define alignment as a macro

// Safe memset to zero out memory
static void zeromem(volatile unsigned char *buf, size_t len) {
  if (buf != NULL && len > 0) {
    memset((unsigned char *) buf, 0, len);
  }
}
...
static size_t roundup(size_t size, size_t align) {
  ...
  // Calculate rounded size, checking for overflow
  ...
  if (remainder == 0) {
    ...
    if (size > SIZE_MAX - padding) {  // Overflow check
      return SIZE_MAX;                // Or indicate error appropriately
    }
...
// Check for size_t overflow in addition
static int is_overflow_add(size_t a, size_t b) {
    return b > SIZE_MAX - a;
}

// Validate pointer is likely valid (not NULL, not obviously corrupt)
static int is_valid_ptr(const void *ptr) {
    if (ptr == NULL) return 0;
    // Check if pointer looks corrupted (very low or very high address)
    uintptr_t addr = (uintptr_t)ptr;
    if (addr < 0x10000) return 0;  // NULL-ish
    #if defined(_WIN32)
    // On Windows, user-space addresses are typically below 0x00007FFFFFFFFFFF
    if (addr > 0x00007FFFFFFFFFFF) return 0;
    #endif
    return 1;
}

// Validate iobuf structure integrity
static int validate_iobuf(const struct mg_iobuf *io, const char *caller) {
    if (io == NULL) {
        LOG(LL_ERROR, ("%s: NULL iobuf", caller));
        return 0;
    }
    if (io->buf != NULL && !is_valid_ptr(io->buf)) {
        LOG(LL_ERROR, ("%s: Corrupted buf pointer %p", caller, io->buf));
        return 0;
    }
    if (io->buf == NULL && io->size != 0) {
        LOG(LL_INFO, ("%s: NULL buf but size=%lu, correcting", caller, (unsigned long)io->size));
        // Don't return error, just correct it
        ((struct mg_iobuf *)io)->size = 0;
        ((struct mg_iobuf *)io)->len = 0;
    }
    if (io->len > io->size) {
        LOG(LL_INFO, ("%s: len(%lu) > size(%lu), correcting", caller, (unsigned long)io->len, (unsigned long)io->size));
        // Don't return error, just correct it
        ((struct mg_iobuf *)io)->len = io->size;
    }
    return 1;
}

#if MG_ENABLE_ASSERT
#define MG_ASSERT(expr) assert(expr)
#else
#define MG_ASSERT(expr) ((void) 0)
#endif
// --------------------- REVISED IOBUF FUNCTIONS ---------------------

// Safely resize a mg_iobuf structure
// Returns 1 on success, 0 on failure.
// IMPORTANT: This function is NOT thread-safe. Caller must provide external
// synchronization (e.g., mutex) if the buffer is accessed from multiple threads.
int mg_iobuf_resize(struct mg_iobuf *io, size_t new_size) {
  // CRITICAL: Validate buffer integrity at entry
  if (!validate_iobuf(io, "mg_iobuf_resize_entry")) {
    return 0;
  }

  // Handle free case (new_size == 0)
  if (new_size == 0) {
    if (io->buf != NULL) {
      // CRITICAL FIX: Skip zeromem to avoid heap corruption
      // If io->size doesn't match actual allocated size, zeroing would corrupt heap
      unsigned char *temp = io->buf;

      // Set buf to NULL FIRST to signal buffer is being freed
      // This is safer than setting size=0 first, because code that checks
      // (buf == NULL) will skip operations, while code seeing (buf != NULL, size=0)
      // might try to use the buffer incorrectly
      io->buf = NULL;  // Set to NULL BEFORE free to prevent double-free
      io->size = 0;
      io->len = 0;

      // CRITICAL: Validate pointer before freeing to prevent heap corruption
      if (is_valid_ptr(temp)) {
        free(temp);  // Free the buffer
      } else {
        LOG(LL_ERROR, ("mg_iobuf_resize: Invalid pointer %p, skipping free", temp));
      }
    } else {
      // Already NULL, ensure size/len are zero
      io->size = 0;
      io->len = 0;
    }
    return 1;  // Success (buffer is now freed/empty)
  }

  // Ensure new_size is aligned, checking for overflow during roundup
  size_t aligned_size = roundup(new_size, IOBUF_ALIGNMENT);
  if (aligned_size < new_size ||
      aligned_size == SIZE_MAX) {  // Check overflow from roundup
    // If roundup returned SIZE_MAX due to overflow or near-limit input
    // Check if the original requested new_size was already SIZE_MAX
    if (new_size == SIZE_MAX &&
        roundup(new_size, IOBUF_ALIGNMENT) == SIZE_MAX) {
      // Allow SIZE_MAX if alignment doesn't change it
      aligned_size = SIZE_MAX;
    } else {
      LOG(LL_ERROR, ("Size overflow during alignment calculation for %lu",
                     (unsigned long) new_size));
      return 0;
    }
  }

  // Only resize if the aligned size is actually different
  if (aligned_size == io->size) {
  ...
  if (aligned_size > io->size) {
    // --- Expanding buffer ---
    // CRITICAL FIX: Save old buffer pointer FIRST, before any operations
    // This prevents race where another thread sets io->buf = NULL between our check and use
    unsigned char *old_buf = io->buf;
    size_t old_size = io->size;
    size_t old_len = io->len;

    // Use calloc for zeroed memory (safer)
    p = calloc(1, aligned_size);
    if (p == NULL) {
      LOG(LL_ERROR,
          ("Failed to allocate %lu bytes", (unsigned long) aligned_size));
      return 0;  // Allocation failed
    }

    // Copy existing data if any (use saved pointers to avoid race conditions)
    if (old_buf != NULL && old_len > 0 && is_valid_ptr(old_buf)) {
      // Defensively ensure we don't copy more than the old size
      size_t copy_len = (old_len <= old_size) ? old_len : old_size;
      if (copy_len > 0 && copy_len <= aligned_size && copy_len <= old_size) {
        // CRITICAL: Validate before memcpy
        if (is_valid_ptr(p)) {
          memcpy(p, old_buf, copy_len);
        } else {
          LOG(LL_ERROR, ("mg_iobuf_resize: Invalid dest pointer %p", p));
          free(p);  // Free newly allocated buffer
          return 0;
        }
      }
    }

    // Free the old buffer *after* successful copy
    // Update io->buf ATOMICALLY to prevent NULL pointer window

    // Update to new buffer FIRST (before freeing old) to prevent NULL pointer window
    io->buf = (unsigned char *) p;
    io->size = aligned_size;
    // io->len remains unchanged as we only copied existing valid data

    // Now free the old buffer after the new one is active
    if (old_buf != NULL && is_valid_ptr(old_buf)) {
      // CRITICAL FIX: Skip zeromem to avoid heap corruption if old_size is wrong
      // If old_size doesn't match actual allocated size (due to corruption),
      // zeroing would write past buffer boundary and corrupt heap
      free(old_buf);
    } else if (old_buf != NULL) {
      LOG(LL_ERROR, ("mg_iobuf_resize: Invalid old_buf %p, skipping free", old_buf));
    }

  } else {
    // --- Shrinking buffer ---
    // MG_ASSERT(aligned_size < io->size);

    if (io->buf == NULL) {
      // Should not happen if io->size > 0, but handle defensively
      LOG(LL_ERROR, ("Cannot shrink NULL buffer with non-zero old size"));
      io->size = 0;  // Correct state
      io->len = 0;
      return 0;  // Indicate error
    }

    // Calculate new length if it needs to be truncated
    size_t new_len = io->len;
    if (io->len > aligned_size) {
      // CRITICAL FIX: Skip zeromem to prevent heap corruption
      // If io->size is wrong, zeroing could write past buffer and corrupt heap
      new_len = aligned_size;
    }

    // CRITICAL FIX: Use malloc/free instead of realloc to avoid heap corruption
    // realloc can corrupt heap if size metadata is inconsistent
    // Allocate new smaller buffer
    p = calloc(1, aligned_size);
    if (p == NULL) {
      LOG(LL_ERROR, ("Failed to allocate %lu bytes for shrinking",
                     (unsigned long) aligned_size));
      io->len = new_len;  // Update length even if allocation failed
      return 0;  // Allocation failed
    }

    // Copy data to new buffer
    size_t copy_len = (new_len <= aligned_size) ? new_len : aligned_size;
    if (copy_len > 0 && io->buf != NULL && is_valid_ptr(io->buf)) {
      if (copy_len <= io->size && is_valid_ptr(p)) {
        memcpy(p, io->buf, copy_len);
      } else {
        LOG(LL_ERROR, ("mg_iobuf_resize_shrink: Invalid memcpy params copy_len=%lu size=%lu",
                       (unsigned long)copy_len, (unsigned long)io->size));
        free(p);
        return 0;
      }
    }
    ...
    // Free old buffer after updating pointers
    if (old_buf != NULL && is_valid_ptr(old_buf)) {
      free(old_buf);
    } else if (old_buf != NULL) {
      LOG(LL_ERROR, ("mg_iobuf_resize_shrink: Invalid old_buf %p, skipping free", old_buf));
    }
  }

  // CRITICAL: Validate buffer integrity at exit
  if (!validate_iobuf(io, "mg_iobuf_resize_exit")) {
    LOG(LL_ERROR, ("mg_iobuf_resize: Buffer corrupted at exit!"));
    // Don't return error, just log - buffer state was set
  }

  return 1;  // Success
}

// Initialize a mg_iobuf structure
// Returns 1 on success, 0 on failure.
int mg_iobuf_init(struct mg_iobuf *io, size_t initial_size) {
  // Validate input
  if (io == NULL) {
    LOG(LL_ERROR, ("mg_iobuf_init: NULL io pointer"));
    return 0;
  }

  // Initialize the structure members reliably
  io->buf = NULL;
  io->size = 0;
  io->len = 0;

  // Use resize to perform initial allocation (if initial_size > 0)
  // mg_iobuf_resize handles the size=0 case correctly (sets buf=NULL, size=0,
  // len=0)
  return mg_iobuf_resize(io, initial_size);
}

// Add data to a mg_iobuf at a specific offset
// Returns number of bytes added (len), or 0 on failure/error.
// Special case: if buf is NULL and len > 0, this reserves space without copying data
size_t mg_iobuf_add(struct mg_iobuf *io, size_t ofs, const void *buf,
                    size_t len) {
  // Validate input
  if (io == NULL) {
    LOG(LL_ERROR, ("mg_iobuf_add: NULL io pointer"));
    return 0;
  }

  // CRITICAL SAFETY: Validate buffer structure integrity before operations
  // If len > size, this indicates corruption or a transient race condition.
  // Instead of failing the operation, correct it and continue.
  if (io->buf != NULL && io->len > io->size) {
    LOG(LL_INFO, ("mg_iobuf_add: Inconsistent state: len(%lu) > size(%lu), correcting",
                   (unsigned long) io->len, (unsigned long) io->size));
    io->len = io->size;  // Cap len to size, don't fail the operation
  }

  // Adding zero bytes is a no-op, return success (0 bytes added)
  if (len == 0) {
    return 0;
  }
  
  // SPECIAL CASE: buf == NULL with len > 0 means "reserve space without copying"
  // This is used by WebSocket operations (mg_ws_wrap) to allocate buffer space
  // that will be filled manually later. We'll zero the space for security.
  int reserve_only = (buf == NULL);

  // --- Check for potential overflows before calculations ---
  if (is_overflow_add(ofs, len)) {
    LOG(LL_ERROR, ("mg_iobuf_add: Offset + length overflow: %lu + %lu",
                   (unsigned long) ofs, (unsigned long) len));
    return 0;
  }

  // --- Calculate minimum size needed ---
  size_t min_size_needed = ofs + len;

  // --- Resize if necessary ---
  if (min_size_needed > io->size) {
    // Calculate desired new size with room to grow (e.g., 25% extra, capped by
    // SIZE_MAX)
    size_t new_size = min_size_needed;
    size_t extra = min_size_needed / 4;  // ~25% growth factor
    if (!is_overflow_add(min_size_needed, extra)) {
      new_size = min_size_needed + extra;
    } else {
      new_size = SIZE_MAX;  // Grow as much as possible if addition overflows
    }

    // Ensure new_size is not less than required (can happen if min_size_needed
    // was already huge)
    if (new_size < min_size_needed) {
      new_size = min_size_needed;
    }

    // Resize (mg_iobuf_resize handles alignment and actual allocation)
    if (!mg_iobuf_resize(io, new_size)) {
      LOG(LL_ERROR, ("mg_iobuf_add: Failed to resize buffer to %lu bytes",
                     (unsigned long) new_size));
      // io state might be inconsistent after failed resize, return 0
      return 0;
    }
    // Post-resize sanity check: Ensure resize achieved required size
    MG_ASSERT(io->size >= min_size_needed);
    if (io->size < min_size_needed) {
      LOG(LL_ERROR,
          ("mg_iobuf_add: Resize inconsistency. Expected >= %lu, got %lu",
           (unsigned long) min_size_needed, (unsigned long) io->size));
      return 0;  // Resize didn't provide enough space
    }
  }

  // --- Prepare for data insertion/append ---

  // Sanity check: Ensure buffer pointer is valid after potential resize
  if (io->buf == NULL && min_size_needed > 0) {
    LOG(LL_ERROR, ("mg_iobuf_add: Buffer is NULL but needed size %lu > 0",
                   (unsigned long) min_size_needed));
    return 0;  // Should not happen if resize succeeded for non-zero size
  }

  // Sanity check: Ensure offset is within the *current* buffer size.
  // It might be > io->len if inserting into allocated but unused space.
  if (ofs > io->size) {
    LOG(LL_ERROR, ("mg_iobuf_add: Offset %lu exceeds buffer size %lu",
                   (unsigned long) ofs, (unsigned long) io->size));
    return 0;
  }

  // Sanity check: Ensure the write operation (ofs + len) will fit within the
  // allocated size. This is the most critical check before memcpy/memmove.
  MG_ASSERT(ofs <= io->size);        // Checked above
  MG_ASSERT(len <= io->size - ofs);  // Combined check for ofs+len <= io->size
  if (len > io->size - ofs) {        // Check for ofs + len > io->size without
                                     // intermediate overflow
    LOG(LL_ERROR,
        ("mg_iobuf_add: Write would exceed buffer size. ofs=%lu, len=%lu, "
         "size=%lu",
         (unsigned long) ofs, (unsigned long) len, (unsigned long) io->size));
    // This indicates a logic error, possibly concurrency or failed resize
    // logic.
    return 0;
  }

  // --- Perform data copy/move ---
  // CRITICAL FIX: Save buffer pointer to prevent race where another thread
  // frees the buffer between our validation and use
  unsigned char *work_buf = io->buf;
  size_t work_len = io->len;

  // Double-check buffer is still valid
  if (work_buf == NULL) {
    LOG(LL_ERROR, ("mg_iobuf_add: Buffer became NULL after validation"));
    return 0;
  }

  // If inserting *before* the current end of data (ofs < work_len), move
  // existing data
  if (ofs < work_len) {
    // We already confirmed `ofs + len <= io->size`.
    // Need to ensure the source `work_buf + ofs` and destination `work_buf + ofs
    // + len` and the amount to move `work_len - ofs` are valid.
    size_t move_count = work_len - ofs;
    MG_ASSERT(ofs + len + move_count <= io->size);  // Check if total fits

    // Use memmove as source and destination overlap
    memmove(work_buf + ofs + len, work_buf + ofs, move_count);
  }

  // Copy the data (for normal mode only)
  if (!reserve_only) {
    // Normal mode: copy the data
    // buf is guaranteed non-NULL here if len > 0
    memcpy(work_buf + ofs, buf, len);
  }
  // CRITICAL FIX: Skip zeromem in reserve_only mode to prevent heap corruption
  // If size metadata is wrong, zeroing could write past buffer and corrupt heap
  // Note: New allocations from calloc are already zeroed; existing buffer space
  // may contain old data but that's acceptable to avoid corruption risk

  // --- Update buffer length ---
  // The new length will be the maximum of the previous length
  // and the position one past the newly added data (ofs + len).
  size_t potential_new_len = ofs + len;
  if (potential_new_len > io->len) {
    io->len = potential_new_len;
  }
  // If we inserted data before the end (ofs < io->len),
  // the length might increase by less than 'len' or stay the same
  // if the insertion point plus length doesn't exceed the original length.
  // Example: io->len=10, ofs=2, len=3 -> new_len = max(10, 2+3) = 10
  // Example: io->len=10, ofs=8, len=5 -> new_len = max(10, 8+5) = 13
  // The calculation `(ofs + len > io->len) ? (ofs + len) : io->len` was
  // correct.

  // Final sanity check: Ensure new length doesn't exceed capacity.
  // This *should* always pass due to the checks before memcpy/memmove.
  // If this fails, it strongly points to a race condition or memory corruption.
  if (io->len > io->size) {
    LOG(LL_ERROR, ("CRITICAL: io->len (%lu) > io->size (%lu) after add "
                   "operation. Race condition or corruption likely.",
                   (unsigned long) io->len, (unsigned long) io->size));
    // What to do? Capping io->len hides the error. Crashing might be better.
    // For now, cap it as the original code did, but log critically.
    io->len = io->size;
    // Consider returning 0 here to signal failure more strongly,
    // but that changes behavior from the original code.
    // return 0;
  }
...
// Delete data from a mg_iobuf at a specific offset
// Returns number of bytes deleted, or 0 on failure/error.
size_t mg_iobuf_del(struct mg_iobuf *io, size_t ofs, size_t len) {
  // Validate input
  if (io == NULL || io->buf == NULL) {
    return 0;  // Nothing to delete from
  }

  // CRITICAL SAFETY: Validate buffer structure integrity
  if (io->len > io->size) {
    LOG(LL_ERROR, ("mg_iobuf_del: Corrupted buffer state: len(%lu) > size(%lu)",
                   (unsigned long) io->len, (unsigned long) io->size));
    return 0;  // Buffer is corrupted, don't touch it
  }

  // Validate offset
  if (ofs >= io->len) {
    return 0;  // Offset is at or past the end of valid data
  }

  // If len is 0, nothing to delete
  if (len == 0) {
    return 0;
  }

  // Calculate effective length to delete: don't delete past the end of valid
  // data
  size_t effective_len = len;
  // Fix: Check for overflow safely by comparing ofs + effective_len directly
  // We need to check if ofs + effective_len > io->len, but avoid overflow
  // The original check "ofs > io->len - effective_len" can underflow when effective_len > io->len
  if (effective_len > io->len - ofs) {  // Safe check: if len is too large
    effective_len = io->len - ofs;
  }

  // If effective length is 0 after adjustment, nothing to delete
  if (effective_len == 0) {
    return 0;
  }

  MG_ASSERT(ofs + effective_len <= io->len);

  // CRITICAL FIX: Save buffer pointer atomically to prevent race conditions
  unsigned char *work_buf = io->buf;
  size_t work_len = io->len;
  size_t work_size = io->size;

  // Double-check buffer is still valid
  if (work_buf == NULL) {
    LOG(LL_ERROR, ("mg_iobuf_del: Buffer became NULL after validation"));
    return 0;
  }

  // Calculate remaining data length after the deleted section
  size_t tail_len = work_len - (ofs + effective_len);

  // If there is data after the deleted section, move it forward
  if (tail_len > 0) {
    // Use memmove as source and destination overlap
    memmove(work_buf + ofs, work_buf + ofs + effective_len, tail_len);
  }

  // CRITICAL FIX: Skip zeromem to prevent heap corruption
  // If work_size is incorrect, zeroing could write past buffer and corrupt heap
  // Security hygiene (zeroing deleted data) is less important than stability

  // Update the length
  io->len -= effective_len;

  // Optional: Consider shrinking the buffer if len becomes much smaller than
  // size if (io->size > SOME_THRESHOLD && io->len < io->size / SHRINK_FACTOR) {
  //    mg_iobuf_resize(io, io->len + SOME_SLACK); // Be careful about resize
  //    failures
  // }

  return effective_len;  // Return the number of bytes actually deleted
}

// Free a mg_iobuf (wrapper around resize to 0)
void mg_iobuf_free(struct mg_iobuf *io) {
  if (io != NULL) {
    mg_iobuf_resize(io,
                    0);  // resize(0) handles freeing buf and resetting members
  }
}
...
void mg_mgr_init(struct mg_mgr *mgr) {
  ...
  mgr->pre_accept_cb = NULL;
  mgr->pre_accept_data = NULL;
...
// IMPORTANT: NOT thread-safe. If calling from multiple threads on the same
// connection, caller MUST provide external synchronization (e.g., mutex around
// this call) to protect c->send buffer from concurrent access.
bool mg_send(struct mg_connection *c, const void *buf, size_t len) {
  // CRITICAL: Validate connection pointer before dereferencing
  if (c == NULL) {
    LOG(LL_ERROR, ("mg_send: NULL connection pointer"));
    return false;
  }

  return c->is_udp ? mg_sock_send(c, buf, len) > 0
                   : mg_iobuf_add(&c->send, c->send.len, buf, len);
}
...
static void accept_conn(struct mg_mgr *mgr, struct mg_connection *lsn) {
...
#if defined(_WIN32) && MG_ENABLE_WINSOCK
  // Windows-specific: Use WSAAccept with condition function
  if (mgr->pre_accept_cb != NULL) {
    // Create a condition function for WSAAccept
    auto condition_func = [](LPWSABUF lpCallerId, LPWSABUF lpCallerData,
                             LPQOS lpSQOS, LPQOS lpGQOS, LPWSABUF lpCalleeId,
                             LPWSABUF lpCalleeData, GROUP *g,
                             DWORD_PTR dwCallbackData) -> int {
      struct mg_mgr *mgr = (struct mg_mgr *) dwCallbackData;

      if (lpCallerId && lpCallerId->len >= sizeof(struct sockaddr)) {
        struct sockaddr *addr = (struct sockaddr *) lpCallerId->buf;
        socklen_t addr_len = lpCallerId->len;

        if (mgr->pre_accept_cb &&
            !mgr->pre_accept_cb(addr, addr_len, mgr->pre_accept_data)) {
          return CF_REJECT;  // Reject before accepting
        }
      }
      return CF_ACCEPT;
    };

    fd = WSAAccept(FD(lsn), &usa.sa, &sa_len, condition_func, (DWORD_PTR) mgr);
  } else {
    fd = accept(FD(lsn), &usa.sa, &sa_len);
  }
#else
  // Non-Windows: Standard accept then check
  fd = accept(FD(lsn), &usa.sa, &sa_len);

  if (fd != INVALID_SOCKET && mgr->pre_accept_cb != NULL) {
    if (!mgr->pre_accept_cb(&usa.sa, sa_len, mgr->pre_accept_data)) {
      closesocket(fd);
      return;
    }
  }
  ...
  if (fd == INVALID_SOCKET) {
    ...
    // Connection accepted and allocated successfully
    char buf[40];
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
  ...
  mg_timer_poll(now);
  bool locked=false;
  CCORSConnCtx *ctx = nullptr;
  
  for (c = mgr->conns; c != NULL; c = tmp) {
      locked=false;
     ctx= reinterpret_cast <CCORSConnCtx *> (c->fn_data);

      if ((c->is_readable || c->is_writable)  && ctx)
          {
         // ctx->lockData();
          locked=true;
          }
...
void mg_tls_init(struct mg_connection *c, struct mg_tls_opts *opts) {
 ...
 // SSL_set_options(tls->ssl, SSL_OP_NO_TLSv1_1);
  //SSL_set_options(tls->ssl, SSL_OP_NO_TLSv1_3);
  //SSL_set_options(tls->ssl,SSL_OP_ALLOW_NO_DHE_KEX);
  //SSL_set_options(tls->ssl,SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION);
  //SSL_set_options(tls->ssl, SSL_OP_ENABLE_MIDDLEBOX_COMPAT);
  SSL_set_options(tls->ssl, SSL_OP_LEGACY_SERVER_CONNECT);
  SSL_set_options(tls->ssl, SSL_OP_PRIORITIZE_CHACHA);
  SSL_set_options(tls->ssl, SSL_OP_ALL);
  ...
  SSL_CTX_set_cipher_list(tls->ctx, "ECDHE-RSA-AES128-GCM-SHA256:HIGH:MEDIUM:!aNULL:!MD5:!RC4");
  SSL_CTX_set_ciphersuites(tls->ctx,
 ...
 // if (opts->ciphers != NULL) SSL_set_cipher_list(tls->ssl, opts->ciphers);
  ...
  //if (c->is_client && opts->ca == nullptr) {
...
void mg_tls_handshake(struct mg_connection *c) {
  ...
  // Immediately fetch error details from OpenSSL and store in a string
  // Now errorMessages contains a detailed textual report of the error
  // Log the error messages
  std::string errorMessages =
      "OpenSSL error after SSL operation: ";
  char buf[256];      // Buffer for error string
  unsigned long err;  // Error code

  // Fetch all error codes from the error queue
  ...
  // Process the return code from SSL_connect or SSL_accept
  if (rc == 1) {
    LOG(LL_DEBUG, ("%lu success", c->id));
    c->is_tls_hs = 0;
  } else {  // rc <= 0
    ...
    if (code != 0) {
      // Log error with the return code and the detailed error messages
      std::string errorMessage = "tls hs: rc " + std::to_string(rc) + ", err " +
                                 std::to_string(code) + ", " + errorMessages;
      mg_error(c, errorMessage.c_str());
    }
  }
}
...
void mg_tls_free(struct mg_connection *c) {
  ...
  if (c == NULL) {
    //fprintf(stderr, "mg_tls_free error: mg_connection is NULL.\n");
    return;
  }

  struct mg_tls *tls = (struct mg_tls *) c->tls;

  // Check if tls is NULL
  if (tls == NULL) {
   // fprintf(stderr, "mg_tls_free error: mg_tls is NULL.\n");
    return;
  }

  // Free ssl if it's not NULL
  if (tls->ssl != NULL) {
    ERR_clear_error();  // Clear existing error queue before the call

    SSL_free(tls->ssl);

    // Check the error queue after the call
    unsigned long err_code = ERR_get_error();
    if (err_code != 0) {
      char err_msg[120];
      ERR_error_string(err_code, err_msg);
      //fprintf(stderr, "SSL_free error: %s\n", err_msg);
      // Handle the error appropriately
    } else {
      tls->ssl = NULL;  // Set to NULL to avoid double free
    }
  }

  // Free ctx if it's not NULL
  if (tls->ctx != NULL) {
    ERR_clear_error();  // Clear existing error queue before the call
    ...
    if (err_code != 0) {
      char err_msg[120];
      ERR_error_string(err_code, err_msg);
     // fprintf(stderr, "SSL_CTX_free error: %s\n", err_msg);
      // Handle the error appropriately
    } else {
      tls->ctx = NULL;  // Set to NULL to avoid double free
    }
  }
...
char *mg_hexdump(const void *buf, size_t len) {
  // Input validation with safe default returns
  if (buf == NULL) {
    char *result = (char *) calloc(1, 32);
    if (result) snprintf(result, 32, "(null buffer)\n");
    return result;
  }

  // Calculate buffer size conservatively
  // Each byte needs ~4 chars in output (2 for hex, 1 for ASCII, 1 for spacing)
  const size_t bytes_per_line = 16;
  const size_t chars_per_line = 80;  // Conservative estimate

  // Calculate number of lines with overflow protection
  size_t lines = len / bytes_per_line;
  if (len % bytes_per_line != 0) lines++;

  // Protect against overflow in size calculation
  size_t buf_size;
  if (lines > (SIZE_MAX / chars_per_line)) {
    buf_size = SIZE_MAX / 2;  // Use a large but safe size
  } else {
    buf_size = lines * chars_per_line + 100;  // Extra space for safety
  }

  char *dst = (char *) calloc(1, buf_size);
  if (dst == NULL) return NULL;  // Memory allocation failed

  const unsigned char *p = (const unsigned char *) buf;
  size_t i = 0;                     // Current byte in input buffer
  size_t n = 0;                     // Current position in output buffer
  size_t remaining = buf_size - 1;  // Remaining space in output buffer

  // Process the buffer byte by byte
  while (i < len && remaining > 30) {
    // ASCII representation buffer for current line, initialized to zeros
    char line_ascii[17] = {0};

    // Print offset at start of line
    int written = snprintf(dst + n, remaining, "%04x", (int) i);
    ...
    size_t line_start = i;
    for (size_t j = 0; j < bytes_per_line && i < len; j++, i++) {
      // Print hex value with space
      unsigned char byte = p[i];
      written = snprintf(dst + n, remaining, " %02x", byte);
      if (written <= 0 || (size_t) written >= remaining) break;
      n += written;
      remaining -= written;

      // Store ASCII representation
      line_ascii[j] = (byte >= 0x20 && byte <= 0x7e) ? (char) byte : '.';
    }

    // Add padding spaces for incomplete lines
    size_t bytes_in_line = i - line_start;
    if (bytes_in_line < bytes_per_line) {
      for (size_t j = 0; j < (bytes_per_line - bytes_in_line) && remaining > 3;
           j++) {
        written = snprintf(dst + n, remaining, "   ");
        if (written <= 0 || (size_t) written >= remaining) break;
        n += written;
        remaining -= written;
      }
    ...
    if (remaining > 4) {
      written = snprintf(dst + n, remaining, "  %s\n", line_ascii);
      if (written <= 0 || (size_t) written >= remaining) break;
      n += written;
      remaining -= written;
    }
...
static void mg_ws_mask(struct mg_connection *c, size_t len) {
  // Check if c points to a valid object
  if (c == NULL) {
    return;
  }

  // Check if c is a client and c's send buffer isn't null
  if (c->is_client && c->send.buf != NULL) {
    // Check if send buffer has enough data to process
    // CRITICAL FIX: Use overflow-safe check instead of len + 4
    // Original: if (c->send.len < len + 4) which can overflow
    // Safe: check if len > send.len - 4 (with underflow protection)
    if (len > SIZE_MAX - 4) {  // Overflow protection: len + 4 would overflow
      return;
    }
    if (c->send.len < 4 || len > c->send.len - 4) {
      return;
    }
...
size_t mg_ws_wrap(struct mg_connection *c, size_t len, int op) {
  ...
  // CRITICAL FIX: Validate that len doesn't exceed current buffer content
  if (len > c->send.len) {
    LOG(LL_ERROR, ("mg_ws_wrap: len(%lu) > send.len(%lu)",
                   (unsigned long) len, (unsigned long) c->send.len));
    return c->send.len;
  }

  // NOTE: order of operations is important!
  if (!mg_iobuf_add(&c->send, c->send.len, NULL, header_len)) {
    return c->send.len;  // Failed to add header space
  }

  // Double-check buffer validity after resize
  // CRITICAL FIX: Use overflow-safe check instead of len + header_len
  if (c->send.buf == NULL) {
    return c->send.len;
  }

  // Overflow-safe check: ensure send.len >= len + header_len
  if (is_overflow_add(len, header_len)) {
    LOG(LL_ERROR, ("mg_ws_wrap: len + header_len would overflow"));
    return c->send.len;
  }
  if (c->send.len < len + header_len) {
    return c->send.len;
  }

  // CRITICAL FIX: Additional safety checks before pointer arithmetic
  // Ensure len <= send.len (checked above) and header_len doesn't cause underflow
  if (header_len > c->send.len - len) {
    LOG(LL_ERROR, ("mg_ws_wrap: header_len too large for buffer"));
    return c->send.len;
  }
