...
*/
class CAppTemplate extends CWindow {
...
//position.positionX, position.positionY, width, height, data, dataType, filePath, thread
  constructor(positionX, positionY, width, height, data, dataType, filePath, thread) {
    super(positionX, positionY, width, height, apptemplateBody, "AppTemplate", CAppTemplate.getIcon(), true); //use Shadow-DOM by default
  ...
  static getFileHandlers() {
    return [new CContentHandler(null, eDataType.bytes, 'org.gridnetproject.UIdApps.appTemplate'), new CContentHandler(null, eDataType.unsignedInteger, 'org.gridnetproject.UIdApps.appTemplate')];
  }
  /*
  static getFileHandlers() {
  ...
  */
  ...
  static getPackageID() {
    return "org.gridnetproject.UIdApps.appTemplate";
  }

  //Settings Manager Support - BEGIN (Note Initialize() and static member fields at the bottom)
  static getSettings() { //[required]
    return CAppTemplate.sCurrentSettings; //Important: static field [required]
  ...
  static setSettings(sets) {
    ...
    CAppTemplate.sCurrentSettings = sets;
    return true;
  }

  loadSettings() {
    CVMContext.getInstance().getSettingsManager.loadSettings(CAppTemplate.getPackageID());
    return this.activateSetings();
  }

  activateSetings() {

    let sets = CAppTemplate.getSettings();
    let settings = null;
    let pack = null;
    let elem = null;
    ...
    if (sets.getVersion == 1) {
      ...
      if (settings == null || typeof settings === 'undefined') {
        sets = CAppTemplate.getDefaultSettings();
        settings = sets.getData;
      }
    } else return false;
...
*/
  ...
  static getDefaultSettings() {
    ...
    return new CAppSettings(CAppTemplate.getPackageID(), obj);
  }
  ...
  //internal processing queue
  {
    ...
    if (this.loadSettings()) {
      ...
      CAppTemplate.setSettings(this.getDefaultSettings());
...
CAppTemplate.sCurrentSettings = new CAppSettings(CAppTemplate.getPackageID);
