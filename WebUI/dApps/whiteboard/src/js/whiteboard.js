...
class whiteboard  {
  ...
  if (aobj !== null) {
    ...
    if (aobj.parentNode !== undefined) {
       this.newOffset = this.recursive_offset(aobj.parentNode);
    }
  ...
  constructor(readOnlyService, configService, infoService, throttlingService)
  {
    this.mThrottlingService = throttlingService;
    this.mReadOnlyService = readOnlyService;
    ...
    this.previousToolHtmlElem= null; // useful for handling read-only mode
    this.tool= "mouse";
    this.thickness= 4;
    /**
     * @type Point
     ...
     * @type Point
     */
  this.startCoords= new Point(0, 0);
    this.drawFlag= false;
    this.oldGCO= null;
    this.mouseover= false;
    this.lineCap= "round"; //butt, square
    this.backgroundGrid= null;
    this.canvasElement= null;
    this.cursorContainer= null;
    this.imgContainer= null;
    this.svgContainer= null; //For draw prev
    this.mouseOverlay= null;
    this.ownCursor= null;
    this.penSmoothLastCoords= [];
    this.svgLine= null;
    this.svgRect= null;
    this.svgCirle= null;
    this.body=null;
    ...
    this.drawBuffer= [];
    this.undoBuffer= [];
    this.drawId= 0; //Used for undo/redo functions
    this.imgDragActive= false;
    this.latestActiveTextBoxId= false; //The id of the latest clicked Textbox (for font and color change)
    this.pressedKeys= {};
    this.settings= {
        whiteboardId: "0",
        username: "unknown",
    ...
    updateOnResize()
    {
      //changes by PauliX 02.05.21
      //1) dynamic canvas size on resize fix-ups
      //2) dynamic client rect on resize with the underlying texture handled now properly
      //[todo:PauliX:urgent]: fix continous drawing when field of view outside of native window's boundaries.
      let cWH = this.window.getClientHeight;
      let cWW = this.window.getClientWidth;

      const dbCp = JSON.parse(JSON.stringify(this.drawBuffer)); // Copy the buffer
      if(this.mPreviousHeight < cWH ||
        this.mPreviousWidth < cWW)
      {//resize only if previous dimensions smaller than the current client rect
        this.canvas.width =cWW;
        this.canvas.height = cWH; // Set new canvas height
        //^[todo:Paulix:urgent]: add getClientRect function to CWindow use that over here
          // Handel resize
      this.mPreviousHeight = cWH;
      this.mPreviousWidth = cWW;
      this.mouseOverlay[0].style.width = cWW+'px';
      this.mouseOverlay[0].style.height = cWH+'px';
      let elems = $(this.window.getBody).find('.drawingArea');
      elems.css('height',cWH+'px');
      elems.css('width',cWW+'px');

      }


      this.drawBuffer = [];
      this.loadData(dbCp); // draw old content in
    }
    updateOnScroll()
    {
      //reposition toolbar
      let topMargin=10;
      let leftMargin=10;
      let toolbar = $(this.window.getBody).find('#toolbar')[0];
      $(toolbar).css('top',this.window.getBodyRoot.scrollTop+topMargin+'px');
      //$(toolbar).css('left',this.window.getBodyRoot.scrollLeft+leftMargin+'px');

    }
    loadWhiteboard(windowP, newSettings) {
      this.window = windowP;
      this.body = this.window.getBody;
      this.whiteboardContainer = $(this.body).find("#whiteboardContainer")[0];
        ...
        _this.backgroundGrid = $(
            `<div class="drawingArea" style="position: absolute; left:0px; top:0; opacity: 0.2; background-image:url('${_this.settings["backgroundGridUrl"]}'); height: 100%; width: 100%;"></div>`
        );
        // container for background images
        _this.imgContainer = $(
            '<div class="drawingArea" style="position: absolute; left:0px; top:0; width: 100%;"></div>'
        );
        // whiteboard canvas
        _this.canvasElement = $(
            '<canvas class="drawingArea" id="whiteboardCanvas" style="position: absolute; left:0px; top:0; cursor:crosshair;"></canvas>'
        );
        // SVG container holding drawing or moving previews
        _this.svgContainer = $(
            '<svg class="drawingArea" style="position: absolute; top:0px; left:0px;" width="100%"></svg>'
        );
        // drag and drop indicator, hidden by default
        _this.dropIndicator = $(
            '<div class="drawingArea" style="position:absolute; width: 100%; border: 7px dashed gray; text-align: center; top: 0px; left: 0px; color: gray; font-size: 23em; display: none;"><i class="far fa-plus-square" aria-hidden="true"></i></div>'
        );
        ...
        _this.textContainer = $(
            '<div class="textcontainer drawingArea" style="position: absolute; left:0px; top:0;  width: 100%; cursor:text;"></div>'
        );
        // mouse overlay for draw callbacks
        _this.mouseOverlay = $(
            '<div id="mouseOverlay" class="drawingArea" style="cursor:none; position: absolute; left:0px; top:0; width: 100%;"></div>'
        );
        ...
        this.canvas = $(this.body).find("#whiteboardCanvas")[0];
        this.canvas.height = this.window.getHeight;
        this.canvas.width =  this.window.getWidth;
        ...
        _this.mousedown = function (e) {
            ...
            if (this.mReadOnlyServicereadOnlyActive) return;
        ...
        _this.textContainer.on("mousemove touchmove", function (e) {
            ...
            this.mThrottlingService.throttle(currentPos, () => {
    ...
    triggerMouseMove(e) {
        ...
        //let eventPoint = Point.fromEvent(e);
        let currentPos =  new Point(e.originalEvent.layerX, e.originalEvent.layerY);//+this.window.getBodyRoot.scrollTop);

        window.requestAnimationFrame(function () {
            // update position
            //currentPos =  new Point(eventPoint.x, eventPoint.y+this.window.getBodyRoot.scrollTop);
    ...
    dragCanvasRectContent(xf, yf, xt, yt, width, height) {
    ...
    drawPenSmoothLine(coords, color, thickness) {
        var _this = this;
        var xm1 = coords[0];
        var ym1 = coords[1];
        var x0 = coords[2];
    ...
    addTextBox(
        ...
        textBox.on("mousemove touchmove", function (e) {
            ...
            this.mThrottlingService.throttle(newPointerPosition, () => {
    ...
    setTextboxPosition(txId, top, left) {
        $(this.body).find("#" + txId).css({ top: top + "px", left: left + "px" });
    }
    ...
    setTool(tool) {
        this.tool = tool;
        if (this.tool === "text" || this.tool === "stickynote") {
            $(this.body).find(".textBox").addClass("active");
            this.textContainer.appendTo($(this.whiteboardContainer)); //Bring textContainer to the front
        } else {
            $(this.body).find(".textBox").removeClass("active");
            this.mouseOverlay.appendTo($(this.whiteboardContainer));
        }
    ...
    setDrawColor(color) {
        ...
        $(this.body).find("#whiteboardColorpicker").css({ background: color });
        if ((_this.tool == "text" || this.tool === "stickynote") && _this.latestActiveTextBoxId) {
            _this.sendFunction({
    ...
    setTextBackgroundColor(textboxBackgroundColor) {
        ...
        $(this.body).find("#textboxBackgroundColorPicker").css({ background: textboxBackgroundColor });
        if ((_this.tool == "text" || this.tool === "stickynote") && _this.latestActiveTextBoxId) {
            _this.sendFunction({
    ...
    updateSmallestScreenResolution() {
        const { smallestScreenResolution } = this.mInfoService;
        const { showSmallestScreenIndicator } = this.mConfigService;
    ...
    handleEventsAndData(content, isNewData, doneCallback) {
        var _this = this;
        var tool = content["t"];
        var data = content["d"];
        var color = content["c"];
    ...
    getImageDataBase64(options, callback) {
        ...
        brackGroundImg.onload = function () {
            ...
            var textBoxCnt = 0;
            $.each($(this.body).find(".textBox"), function () {
