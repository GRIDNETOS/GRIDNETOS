...
class CWhiteboard extends CWindow {
    ...
    try {
      ...
      switch (wrapper.getEntityType) {
        case this.getProtocolID: //Whiteboard Protocol
          let msg = CWhiteboardMsg.instantiate(wrapper.getData);
          content = msg.getPayload(); //CTools.getInstance().escapeAllContentStrings(msg.getPayload());
          let typeStr = gTools.arrayBufferToString(msg.getType);
          let a = 0;
          switch (typeStr) {
  ...
  broadcast(type, obj) {
    ...
    let msg = new CWhiteboardMsg(type, JSON.stringify(obj));
  ...
  set setSwarm(swarm) {
    ...
    if (swarm != null) {
      this.setTitle(this.mTools.bytesToString(swarm.getID) + ' - ⋮⋮⋮ Whiteboard');
      this.mSwarm.addSwarmConnectionStateChangeEventListener(this.swarmConnectionChangedEventHandler.bind(this), this.mID);
      this.mSwarm.addSwarmConnectionNewDataListener(this.newSwarmDataEventHandler.bind(this), this.mID);
