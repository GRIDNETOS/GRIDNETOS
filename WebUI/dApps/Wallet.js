...
.nav-tabs .dropdown-menu {
    ...
    text-shadow: 0 0 4px #8fffff;'> UNKNOWN</span>
        ...
        <input type="text" autocomplete="off" class="txtRO noselect addressType" id="addressTxt" value="unknown"readonly><button onclick="gTools.copyControlValueToClipboard('#addressTxt');" id="copyAddrBtn" class="buttonStd"><i class="fa fa-copy"></i>
              </button><br>
...
<div  style="white-space:nowrap;">
    ...
    <br/><br/>
        <button id="sendBtn" class="buttonStd" onclick="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val()); if(!window.getCheckSAOk(2)){ window.animateCSS(this.id,'shakeX'); return}; window.issueTransfer(false);"><i class="fa fa-hourglass"></i> Enqueue</button>
        <button id="sendBtnNow" class="buttonStdGreen"  onclick="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val());if(!window.getCheckSAOk(2)){ window.animateCSS(this.id,'shakeX'); return};  window.issueTransfer(true)">Send Now <i class="fa fa-send"></i> </button>
            </div>
      </div>
              </p></div>
    ...
    Extended Info
      ...
      <fieldset class="groupbox-border">
        ...
        <input type="text"   autocomplete="off" class ='txtInput numType' style="color:#0cff00" id="singleTokenValueTxt" value="1" onclick="gTools.copyControlValueToClipboard('#balanceTxt');"  ><br>
            <button id="createTP" class="buttonStd" style='margin-bottom:1em'onclick="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val()); window.createTP(false);"><i class="fa fa-hourglass"></i> Enqueue</button>
        <button id="createTPNow" class="buttonStdGreen"  onclick="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val()); window.createTP(true)">Create Now <i class="fa fa-send"></i> </button>
          </fieldset>
            </div>
            </div></p> </div>
       ...
       <button class="buttonC" onClick ="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val()); if(!window.isSFOk) return; window.cashOutSelected()"style ="margin-top:1em;"type="button">Cash-Out <b>Selected </b></button>

        <button class="buttonC" onClick ="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val()); if(!window.isSFOk) return;  window.cashOutEverything()"style ="margin-top:1em;background-color: #116F50;"type="button">Cash-Out <b>Everything </b></button><br><br>
            </div>
            </div></p> </div>
      ...
      <br/>
      <button id="saveSettingsBtn" onclick="let window = gWindowManager.getWindowByID($($(this).closest('.idContainer')).find('#windowIDField').first().val()); window.saveWalletID();  " class="buttonStd"><i class="fa fa-send"></i> Save Settings </button><br/>
...
class CWallet extends CWindow {
  ...
  issueTransfer(immediateOne) {
    ...
    //-------------State-Less channels support - END
      ...
      //ONLY kernerl-mode UI dApps are allowed to break any pending Commit-Locks by providing the second parameter equal to TRUE.
